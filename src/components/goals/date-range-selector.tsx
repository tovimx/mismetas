...
export function DateRangeSelector({ onChange, className }: DateRangeSelectorProps) {
  ...
  useEffect(() => {
    if (scrollContainerRef.current && selectedIndex >= 0) {
      ...
      if (selectedElement) {
        const scrollLeft =
          selectedElement.offsetLeft - container.clientWidth / 2 + selectedElement.clientWidth / 2;
  ...
  return (
    <div className={cn('space-y-3', className)}>
      ...
      <input type="hidden" name="durationValue" value={dateOptions[selectedIndex]?.value || 1} />
      <input
        type="hidden"
        name="durationUnit"
